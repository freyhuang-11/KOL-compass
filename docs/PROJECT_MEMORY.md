...
- 对客页面、弹窗、按钮、表格、系统消息不得出现实现词：`API发送`、`接口`、`后端`、`payload`、`schema`、`Target Collaboration API`、`Conversation / Message API`、`官方 ID`。必须改成业务词，例如 `待提交`、`提交失败`、`查看提交结果`、`连接服务`、`店铺授权`、`定向邀约编号`。
