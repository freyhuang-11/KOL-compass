...
最近一次心跳检查：2026-06-22 05:11 CST，当前分支已连接 GitHub 远端，`node --check app.js` 与 `node smoke-test.js` 通过。本轮增强平台管理端：商家统计、入驻审批台账、通过/驳回/恢复待审动作、系统消息和操作日志记录；截图 `smoke-admin.png` 已重新生成并目视检查通过。
