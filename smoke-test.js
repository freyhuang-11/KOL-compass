...
async function main() {
  ...
  assert("outreach API submission uses backend instead of manual status changes", ["submitOutreachApi", "apiRequest(\"/api/tiktok/outreach/submit\"", "messageText", "creator_open_id", "shop_cipher", "提交到后端发送"].every((text) => app.includes(text)) && ["/api/tiktok/outreach/submit", "createCreatorConversation", "sendCreatorImMessage", "/affiliate_seller/202508/conversations", "/affiliate_seller/202412/conversations/${conversationId}/messages", "TARGET_COLLABORATION_SCHEMA_REQUIRED"].every((text) => server.includes(text)));
  assert("target collaboration schema blocker is explicit and inspectable", ["isTargetSchemaRequired", "applyTargetSchemaBlock", "openOutreachApiResult", "定向邀约待配置", "查看API结果", "payload_preview"].every((text) => app.includes(text)) && ["TARGET_COLLABORATION_SCHEMA_REQUIRED", "targetCollaborationPayloadPreview"].every((text) => server.includes(text)));
