...
async function main() {
  ...
  assert("target collaboration official API mapping is implemented", ["submitTargetCollaborationForRow", "targetOfficialId", "openOutreachApiResult", "查看API结果"].every((text) => app.includes(text)) && ["createTargetCollaboration", "targetCollaborationRequestBody", "productTargetCommissionRate", "productAdsCommissionRate", "target_commission_rate", "creator_user_open_ids", "seller_contact_info", "free_sample_rule", "/affiliate_seller/202508/target_collaborations"].every((text) => server.includes(text)));
