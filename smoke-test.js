...
async function main() {
  ...
  assert("creator GMV uses TikTok currency without monthly suffix", ["TikTok GMV（接口币种）", "creatorGmvDisplay", "sanitizeGmvValue"].every((text) => app.includes(text) || server.includes(text)) && !["$10K/月", "$10K-$50K", ">$500K"].some((text) => app.includes(text)));
