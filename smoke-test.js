...
async function main() {
  ...
  assert("platform creator auto import keeps running with five second polling and backoff", ["KOL_CREATOR_AUTO_IMPORT_INTERVAL_MS || 5 * 1000", "KOL_CREATOR_AUTO_IMPORT_INITIAL_DELAY_MS || 5 * 1000", "KOL_CREATOR_SEARCH_PAGE_DELAY_MS || 5000", "KOL_CREATOR_RATE_LIMIT_BACKOFF_MS || \"60000,120000,300000,600000\"", "Math.max(5_000, CREATOR_AUTO_IMPORT_INTERVAL_MS)", "isTikTokRateLimitError", "nextRunAt", "rateLimitCount", "creatorBackoffMs"].every((text) => server.includes(text)));
