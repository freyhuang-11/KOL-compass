...
async function main() {
  ...
  assert("TikTok creator search API wired", ["/api/tiktok/creators/search", "/affiliate_seller/202508/marketplace_creators/search", "normalizeCreators"].every((text) => server.includes(text)) && ["apiRequest(\"/api/tiktok/creators/search\"", "达人抓取失败"].every((text) => app.includes(text)));
  assert("TikTok creator fetch is automatic for customers", ["autoImportCreatorsForCurrentMarket", "creatorAutoImportInFlight", "店铺绑定后自动抓取达人", "if (state.page === \"kol\")"].every((text) => app.includes(text)));
  ...
  assert("shop authorization auto-syncs products and creators", app.includes("await syncProducts({ silent: true })") && app.includes("await syncCreators({ silent: true, reason: \"店铺绑定后自动抓取达人\" })"));
