...
async function main() {
  ...
  assert("creator average video metrics are separate table fields", ["avgVideoViews", "avgLiveUv", "均播/直播UV", "metric-stack"].every((text) => app.includes(text) || read("app.css").includes(text)) && ["avg_ec_video_view_count", "avg_ec_live_uv"].every((text) => server.includes(text)));
